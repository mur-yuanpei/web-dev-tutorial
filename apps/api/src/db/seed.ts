...
async function main() {
  // 幂等：默认做**增量升级** —— 按 slug 匹配，已存在的课程完全不动，
  // SEED 数组里新出现的课程追加到 max(order)+1 之后。
  // 这样每次部署都能无脑跑一遍：老课程原样保留，新课程自动补齐。
  //
  // --force：清库重灌（老行为），用于本地 db:reset、或需要刷新已有课程内容时
...
/** 增量升级：按 slug 跳过已有课程，只追加 SEED 里新出现的课程 */
async function incrementalSeed() {
  ...
  const toInsert = SEED.filter((c) => !existingSlugs.has(c.slug));

  if (toInsert.length === 0) {
    console.log(`✔ 无新增课程（数据库已有 ${existing.length} 门）`);
    console.log("  如需修改已有课程内容，请重跑 seed 时加 --force（会清空重灌）");
    return;
  }

  console.log(`🆕 增量升级：追加 ${toInsert.length} 门新课程`);
  for (const course of toInsert) {
    await insertCourse(course, nextOrder++);
  }
  console.log("✅ 增量 seed 完成");
}

/** 清库重灌：完整按 SEED 数组顺序重建 */
async function forceReseed() {
  console.log("🌱 --force 模式：清空并重灌全部教程内容");
  await db.delete(courses);
  console.log("  已清空旧数据");

  for (const [courseIdx, course] of SEED.entries()) {
    await insertCourse(course, courseIdx);
  }
  console.log("✅ Seed 完成");
}

/** 插入一门课程 + 它的所有 chapters + sections */
async function insertCourse(course: SeedCourse, order: number) {
